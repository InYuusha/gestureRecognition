...
def find_face(img): #检测并标识人脸及眼睛
    ...
    face_cascade = cv2.CascadeClassifier('/home/inyuusha/Project Arduino/demo/cascade/haarcascade_frontalface_alt.xml') #调用人脸识别模型，默认安装
    eye_cascade = cv2.CascadeClassifier('/home/inyuusha/Project Arduino/demo/cascade/haarcascade_eye.xml') #调用眼睛识别模型，默认安装
...
def find_palm(img): #检测并标识手掌手势
    ...
    fist_cascade = cv2.CascadeClassifier('/home/inyuusha/Project Arduino/demo/xml/palm.xml')
...
def find_thumb(img): #检测并标识大拇指手势
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    fist_cascade = cv2.CascadeClassifier("/home/inyuusha/Project Arduino/demo/xml/thumb.xml")
